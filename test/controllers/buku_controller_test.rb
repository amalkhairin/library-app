...
class BukuControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create a new book if admin' do
    assert_difference('Buku.count', 1) do
      post '/buku',
           params: { buku: { barcode: '142352525', isbn: '25252525-23', judul: 'Hai Dunia', penulis: 'Fulan', jumlah_buku: 1, is_available: true, deskripsi: "Lorem Ipsum", penerbit: "Generasi Gigih" } }, headers: { HTTP_AUTHORIZATION: "JWT #{@admin_token}" }
    end
  ...
  test 'should not create a new book if not admin' do
    assert_no_difference('Buku.count') do
      post '/buku',
           params: { buku: { barcode: '142352525', isbn: '25252525-23', judul: 'Hai Dunia', penulis: 'Fulan', jumlah_buku: 1, is_available: true } }, headers: { HTTP_AUTHORIZATION: "JWT #{@user_token}" }
    end
