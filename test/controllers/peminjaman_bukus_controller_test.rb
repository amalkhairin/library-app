...
class PeminjamanBukusControllerTest < ActionDispatch::IntegrationTest
  setup do
    ...
    @admin = User.create(name: "Cristiano Ronaldinho", username: 'ronaldinho', email: 'ronaldinho@gmail.com', telephone: '0888888889', password: 'admin1', role_id: 1)
  ...
  test 'user can not borrow book if book not available' do
    assert_no_difference('@user.peminjaman_bukus.count') do 
      post "/buku/#{@book2.id}/peminjaman_buku", params: {buku_id: @book2.id}, headers: {HTTP_AUTHORIZATION: "JWT #{@user_token}"}
    end
